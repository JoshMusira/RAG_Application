...
from .. import models
from src.rag import get_answer_and_docs, get_answer_and_docs_combined
...
async def ChatCombined(message: models.Message):
    try:
        response = get_answer_and_docs_combined(message.message)
        response_content = {
            "question": message.message,
            "answer": response["answer"],
            "documents": [doc.dict() for doc in response["context"]],
